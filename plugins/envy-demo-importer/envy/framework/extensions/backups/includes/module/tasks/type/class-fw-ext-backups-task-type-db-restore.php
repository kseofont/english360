...
class FW_Ext_Backups_Task_Type_DB_Restore extends FW_Ext_Backups_Task_Type {
	...
	private function do_replace(array $args, array $state) {
		...
		$rename_sql       = array();
		$drop_sql         = array();
		$drop_table_names = array(); // plain names without backticks, for FK comparisons

		foreach ($state['tables'] as $name => $restored) {
			if ($restored) {
				if (isset($current_tables[$name])) {
					// collect names for drop; keep both raw and backticked variants
					$drop_table_names[] = $wpdb->prefix . $name; // raw for FK lookup
				}
		...
		if (!empty($rename_sql)) {
			if (!empty($drop_table_names)) {

				$foreigns = $wpdb->get_results( "SELECT constraint_name, column_name, referenced_table_name, referenced_column_name, table_name FROM information_schema.key_column_usage WHERE TABLE_SCHEMA='{$wpdb->dbname}' AND referenced_table_name IS NOT NULL", ARRAY_A );

				foreach ( $foreigns as $foreign ) {
					if ( in_array( $foreign['referenced_table_name'], $drop_table_names, true ) ) {
						$wpdb->query( "ALTER TABLE `{$foreign['table_name']}` DROP FOREIGN KEY `{$foreign['constraint_name']}`" );
					}
				}

				// build quoted DROP TABLE statement
				$drop_sql = "DROP TABLE \n" . implode(
					" , \n",
					array_map(function($t){ return '`' . esc_sql($t) . '`'; }, $drop_table_names)
				);

				// temporarily disable foreign key checks to avoid residual FK issues
				$wpdb->query('SET FOREIGN_KEY_CHECKS=0');
				...
				// keep FK checks disabled until rename completes
			}
			{
				...
				// re-enable foreign key checks after drop/rename
