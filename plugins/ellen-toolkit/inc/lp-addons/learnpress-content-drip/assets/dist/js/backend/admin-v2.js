...
const initTomSelect = () => {
  const contentDripEl = document.querySelector('#learn-press-content-drip');
  if (!contentDripEl) {
    return;
  }
  const tomSelectEls = Array.prototype.slice.call(contentDripEl.querySelectorAll('.drip-prerequisite-items'));
  if (tomSelectEls.length) {
    tomSelectEls.map(tomSelectEl => {
      ...
      new (tom_select__WEBPACK_IMPORTED_MODULE_0___default())(tomSelectEl, {
        ...dataStruct.setting
      });
    });
  }
...
//Revert jquery to javascript in admin.js
const LP_Content_Drip_Admin = function () {
  //Popup form
  const quickSettingEl = document.querySelector('.quick-settings');
  if (quickSettingEl) {
    const iconEl = document.querySelector('.quick-settings .icon');
    const form = quickSettingEl.querySelector('.quick-settings-form');
    const closeEl = quickSettingEl.querySelector('.close-quick-settings');
    const applyEl = quickSettingEl.querySelector('.apply-quick-settings');
    if (!form || !closeEl || !applyEl) {
      return;
    }
    ...
    applyEl.addEventListener('click', () => {
      const start = parseFloat(form.querySelector('input[name="start"]').value);
      const step = parseFloat(form.querySelector('input[name="step"]').value);
      const type = form.querySelector('select[name="type"]').value;
      const dripItems = document.querySelectorAll('.item-delay');
      let i = start;
      dripItems.forEach(function (item) {
        const delayType = item.querySelector('.delay-type');
        delayType.value = 'interval';
        const delayInterval0 = item.querySelector('.delay-interval-0');
        const delayInterval1 = item.querySelector('.delay-interval-1');
        delayInterval0.value = i;
        delayInterval1.value = type;
        item.classList.remove('immediately', 'interval');
        item.classList.add('interval');
        const specific = item.querySelector('.delay-specific-datetimepicker');
        if (specific) {
          specific.value = '';
          item.classList.remove('specific');
        }
        i += step;
      });
    });
  ...
  //Set class item-delay
  const itemDelayEls = Array.prototype.slice.call(document.querySelectorAll('.item-delay .delay-type'));
  if (itemDelayEls.length) {
    itemDelayEls.map(itemDelayEl => {
      itemDelayEl.addEventListener('change', function () {
        const parent = itemDelayEl.parentElement;
        parent.classList.remove('immediately', 'interval', 'specific');
        parent.classList.add(itemDelayEl.value);
      });
    });
  }
};
