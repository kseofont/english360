...
class WC_Twilio_SMS_Admin {
	...
	public function enqueue_scripts_and_styles() {
		...
		if ( $is_order_edit_screen || ( $current_screen && 'product' === $current_screen->id ) || wc_twilio_sms()->is_plugin_settings() ) {
			...
			wp_localize_script( 'wc-twilio-sms-admin', 'wc_twilio_sms_admin', array(
				...
				// Edit order screen
				'edit_order_id'              => Framework\SV_WC_Order_Compatibility::get_order_id_for_order_edit_screen(),
	...
	public function display_order_meta_box( $post_or_order_object ) {
		...
		if ( $post_or_order_object instanceof \WP_Post ) {
			$optin = get_post_meta( $post_or_order_object->ID, OptInCheckbox::META_KEY, true );
		} elseif ( $post_or_order_object instanceof \WC_Order ) {
			$optin = $post_or_order_object->get_meta( OptInCheckbox::META_KEY );
		} else {
			$optin = 0;
		}
		...
		<p style="margin-bottom:20px;padding-bottom:20px;border-bottom:1px solid #eee;">
			...
			<label for="wc_twilio_sms_toggle_order_updates"><?php esc_html_e( 'Send automated order updates.', 'woocommerce-twilio-sms-notifications' ); ?></label>
		</p>

		<?php $default_message = apply_filters( 'wc_twilio_sms_notifications_default_admin_sms_message', '' ); ?>

		<p><?php esc_html_e( 'Send SMS Message:', 'woocommerce-twilio-sms-notifications' ); ?></p>
		...
		<p><a class="button tips" id="wc_twilio_sms_order_send_message" data-tip="<?php esc_attr_e( 'Send an SMS to the billing phone number for this order.', 'woocommerce-twilio-sms-notifications' ); ?>"><?php esc_html_e( 'Send SMS', 'woocommerce-twilio-sms-notifications' ); ?></a>
	...
	public function add_link_field( $field ) {
		...
		?>
			<tr valign="top">
				...
				<td class="forminp">
					<a href="<?php echo esc_url( $field['href'] ); ?>" class="<?php echo esc_attr( $field['class'] ); ?>"><?php echo esc_html( $field['name'] ); ?></a>
				</td>
			</tr>
		<?php
