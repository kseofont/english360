...
class Bookings {
	...
	public function save_booking_notification_tab_options( $product_id ) {
		...
		if ( $product ) {
			...
			foreach ( array_keys( $this->get_booking_notifications() ) as $notification ) {
				...
				if ( isset( $_POST[ "wc_twilio_sms_bookings_{$notification}_override" ] ) && $this->is_valid_product_override_option( sanitize_text_field( $_POST[ "wc_twilio_sms_bookings_{$notification}_override" ] ) ) ) {
	...
	public function output_booking_schedule_field_html( $args ) {
		...
		echo $booking_schedule->get_field_html( $args, true ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
		...
		if (WC()->cart && is_array(WC()->cart->get_cart())) {
			foreach (WC()->cart->get_cart() as $cart_item) {
				...
				if ($cart_item['data'] instanceof \WC_Product && is_wc_booking_product($cart_item['data'])) {
					$bookingsLabel = get_option('wc_twilio_sms_bookings_optin_checkbox_label', __('Please send booking reminders via text message', 'woocommerce-twilio-sms-notifications'));

					if (! empty($bookingsLabel)) {
						$label = $bookingsLabel;
					}
					break;
				}
	...
	public function is_notification_enabled( $booking_id, $notification ) {
		...
		if ( $booking && $product = $booking->get_product() ) {
			...
			if ( ! $this->is_admin_notification( $notification ) && $enabled && $booking->get_order() ) {
				$enabled = '1' === $booking->get_order()->get_meta( OptInCheckbox::META_KEY );
			}
		}
