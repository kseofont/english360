	...
	public function get_field_html( $args, $as_table = false ) {
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for="<?php echo esc_attr( $field_number ) ?>"><?php echo esc_html__( $args['title'], 'woocommerce-twilio-sms-notifications' ); ?></label>
			</th>
			<td>
		<?php else : ?>
			<label for="<?php echo esc_attr( $field_number ) ?>"><?php echo esc_html__( $args['title'], 'woocommerce-twilio-sms-notifications' ); ?></label>
		<?php endif; ?>
			<input
				...
				value="<?php echo esc_attr( $this->number ) ?>"
			...
			<span class="wc-twilio-sms-post-field" style="display: inline-block; margin-left: 1em; vertical-align: bottom;"><i><?php echo esc_html( $args['post_field'] ); ?></i></span>
		<?php
		if ( $as_table ) {
		?>
			</td>
