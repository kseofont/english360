...
class WC_Twilio_SMS_AJAX {
	...
	public function send_test_sms() {
		...
		$this->verify_request( sanitize_text_field( $_POST['security'] ), 'wc_twilio_sms_send_test_sms' );
		...
		try {
			...
			exit( esc_html__( 'Test message sent successfully', 'woocommerce-twilio-sms-notifications' ) );

		} catch ( Exception $e ) {

			die( sprintf( esc_html__( 'Error sending SMS: %s', 'woocommerce-twilio-sms-notifications' ), esc_html( $e->getMessage() ) ) );
		}
	}
	...
	public function toggle_order_updates() {
		...
		$this->verify_request( sanitize_text_field( $_POST['security'] ), 'wc_twilio_sms_toggle_order_updates' );

		$order_id = ( is_numeric( $_POST['order_id'] ) ) ? absint( $_POST['order_id'] ) : null;

		if ( $order_id && $order = wc_get_order( $order_id ) ) {

			$current_status = $order->get_meta( OptInCheckbox::META_KEY );
	...
	public function send_order_sms() {
		...
		$this->verify_request( sanitize_text_field( $_POST['security'] ), 'wc_twilio_sms_send_order_sms' );
		...
		exit( esc_html__( 'Message Sent', 'woocommerce-twilio-sms-notifications' ) );
	}
	...
	private function verify_request( $nonce, $action ) {
		...
		if ( ! is_admin() || ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'woocommerce-twilio-sms-notifications' ) );
		}

		if( ! wp_verify_nonce( $nonce, $action ) ) {
			wp_die( esc_html__( 'You have taken too long, please go back and try again.', 'woocommerce-twilio-sms-notifications' ) );
		}
