...
use SkyVerge\WooCommerce\Twilio_SMS\Blocks\CheckoutBlockIntegration;
use SkyVerge\WooCommerce\Twilio_SMS\Integrations\Bookings;
use SkyVerge\WooCommerce\Twilio_SMS\OptInCheckbox;
use SkyVerge\WooCommerce\Twilio_SMS\BackwardsCompatibility\OrderMetaKeyMigrationHandler;
...
class WC_Twilio_SMS extends Framework\SV_WC_Plugin {
	...
	/** plugin id */
	public const PLUGIN_ID = 'twilio_sms';

	/** plugin text domain */
	const TEXT_DOMAIN = 'woocommerce-twilio-sms-notifications';

	/** @var WC_Twilio_SMS_Admin|null instance */
	protected ?WC_Twilio_SMS_Admin $admin = null;

	/** @var WC_Twilio_SMS_AJAX|null instance */
	protected ?WC_Twilio_SMS_AJAX $ajax = null;

	/** @var WC_Twilio_SMS_API|null instance */
	private ?WC_Twilio_SMS_API $api = null;

	private CheckoutBlockIntegration $block_checkout;

	/** @var Bookings|null instance */
	private ?Bookings $bookings = null;

	private OrderMetaKeyMigrationHandler $orderMetaKeyMigrationHandler;
	...
	/**
	 * Adds hook callback registrations.
	 *
	 * @return void
	 */
	public function addHooks()
	{
		// Register backwards-compat metadata hooks.
		$this->orderMetaKeyMigrationHandler->addHooks();

		// Register block hooks
		$this->block_checkout->addHooks();
	...
	private function includes() {
		...
		// the checkout block integration
		$this->block_checkout = new CheckoutBlockIntegration();

		// opt-in checkbox meta key migration handler
		$this->orderMetaKeyMigrationHandler = new OrderMetaKeyMigrationHandler();
	...
	public function add_opt_in_checkbox() {
		...
		if ( ! empty( $_POST['wc_twilio_sms_optin'] ) ) {
			...
			$value = OptInCheckbox::getDefaultValue();
		}
	...
	public function process_opt_in_checkbox( $order_id ) {
		...
		$order->update_meta_data( OptInCheckbox::META_KEY, 1 );
	...
	public function erase_opt_in( $order ) {
		...
		if ( $order instanceof \WC_Order ) {
			...
			$order->delete_meta_data( OptInCheckbox::META_KEY );
			$order->save_meta_data();
		}
	}
	 ...
	 */
	public static function instance() : WC_Twilio_SMS
	{
 ...
 * @return WC_Twilio_SMS
 */
function wc_twilio_sms() : WC_Twilio_SMS
{
